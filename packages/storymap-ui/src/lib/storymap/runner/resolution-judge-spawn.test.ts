...
describe("o juiz de resolução nasce com teto de custo", () => {
  ...
  it("buildJudgeArgs: default 2 USD; override; null e 0 desligam", () => {
    ...
    expect(budgetOf(buildJudgeArgs(p, "/tmp/nota.md", 1.25).args)).toBe("1.25");
    expect(budgetOf(buildJudgeArgs(p, "/tmp/nota.md", null).args)).toBeNull();
    expect(budgetOf(buildJudgeArgs(p, "/tmp/nota.md", 0).args)).toBeNull();
