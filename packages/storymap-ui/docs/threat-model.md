...
| Suíte escrita por agente lê credencial / fala com o host durante o gate | o SELO: cada comando do delta (suíte, typecheck, `vitest -u`) numa unidade transiente do systemd — root **sem capability**, FS read-only fora da árvore do gate, `/run` e `/tmp` privados, credenciais conhecidas inacessíveis, sem rede por unidade (`network: deny`); fixture hostil prova a contenção no host real | `runner/gate-sandbox.ts`, `gate-sandbox.test.ts` | a sonda prova o selo no host (senão o gate cai para `none` com aviso no log e no preflight); **não** isola PID (`PrivatePIDs` ausente no systemd 255): a unidade vê e pode sinalizar processos do mesmo uid; é deny-list, não allow-list |
